...
export function transformPowerballNumbers(winningNumbers: string): {
  ...
  return {
    main: mainNumbers.map(n => Number.parseInt(n, 10).toString()).join(','),
    secondary: Number.parseInt(powerball, 10).toString(),
  };
}
...
export function transformCash4LifeNumbers(
  ...
  return {
    main: transformSpaceSeparatedNumbers(winningNumbers),
    secondary: Number.parseInt(cashBall, 10).toString(),
  };
}
