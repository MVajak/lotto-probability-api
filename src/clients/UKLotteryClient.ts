...
import {parseCSVRows} from '../common/utils/csv';
import {
...
export class UKLotteryClient {
  ...
  private parseEuroMillionsCSV(csv: string): UKEuroMillionsDrawDto[] {
    ...
    return rows.map(cols => ({
      drawDate: cols[0],
      ball1: parseInt(cols[1], 10),
      ball2: parseInt(cols[2], 10),
      ball3: parseInt(cols[3], 10),
      ball4: parseInt(cols[4], 10),
      ball5: parseInt(cols[5], 10),
      luckyStar1: parseInt(cols[6], 10),
      luckyStar2: parseInt(cols[7], 10),
      // cols[8] is UK Millionaire Maker (skip)
      drawNumber: parseInt(cols[9], 10),
    }));
  }
  ...
  private parseLottoCSV(csv: string): UKLottoDrawDto[] {
    ...
    return rows.map(cols => ({
      ...
      ball4: parseInt(cols[4], 10),
      ball5: parseInt(cols[5], 10),
      ball6: parseInt(cols[6], 10),
      bonusBall: parseInt(cols[7], 10),
      // cols[8-10] are Ball Set, Machine, Raffles (skip)
      drawNumber: parseInt(cols[11], 10),
    }));
  }
  ...
  private parseThunderballCSV(csv: string): UKThunderballDrawDto[] {
    ...
    return rows.map(cols => ({
      ...
      ball4: parseInt(cols[4], 10),
      ball5: parseInt(cols[5], 10),
      thunderball: parseInt(cols[6], 10),
