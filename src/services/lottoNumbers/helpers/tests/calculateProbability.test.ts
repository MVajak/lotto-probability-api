...
describe('calculateProbability', () => {
  describe('calculateNumberStats', () => {
    it('should return stats with correct counts and probabilities', () => {
      ...
      const result = calculateNumberStats(numbers, LottoType.EURO);
    ...
    it('should include all numbers in range, even if not present', () => {
      ...
      expect(result.length).to.equal(EURO_PRIMARY_NUMBER_RANGE.end);
      expect(result.find(r => r.digit === 4)?.count).to.equal(0);
    });

    it('should return probability 0 for all if input is empty', () => {
      const result = calculateNumberStats([], LottoType.EURO);
    ...
    it('should handle BINGO win class and map correct range', () => {
      ...
      const result = calculateNumberStats(
        numbers,
        LottoType.BINGO,
        false,
        BINGO_CENTER_SQUARE_GAME_WIN_CLASS,
      ...
      expect(result.length).to.equal(
        BINGO_CENTER_NUMBER_RANGE.end - BINGO_CENTER_NUMBER_RANGE.start + 1,
      );
    });

    it('should return empty list if range is 0 (invalid type)', () => {
      const result = calculateNumberStats([1, 2, 3], 'UNKNOWN' as LottoType);
    ...
    describe('range path coverage', () => {
      it(`${LottoType.EURO} - should return primary range`, () => {
        const result = calculateNumberStats([1, 2], LottoType.EURO, false);
        expect(result.length).to.equal(EURO_PRIMARY_NUMBER_RANGE.end);
      });

      it(`${LottoType.EURO} - should return secondary range`, () => {
        const result = calculateNumberStats([1, 2], LottoType.EURO, true);
        expect(result.length).to.equal(EURO_SECONDARY_NUMBER_RANGE.end);
      });

      it(`${LottoType.KENO} - should return primary range`, () => {
        const result = calculateNumberStats([1], LottoType.KENO);
        expect(result.length).to.equal(KENO_PRIMARY_NUMBER_RANGE.end);
      });

      it(`${LottoType.VIKINGLOTTO} - should return primary range`, () => {
        const result = calculateNumberStats([1], LottoType.VIKINGLOTTO, false);
        expect(result.length).to.equal(VIKING_PRIMARY_NUMBER_RANGE.end);
      });

      it(`${LottoType.VIKINGLOTTO} - should return secondary range`, () => {
        const result = calculateNumberStats([1], LottoType.VIKINGLOTTO, true);
        expect(result.length).to.equal(VIKING_SECONDARY_NUMBER_RANGE.end);
      });
      ...
      it(`${LottoType.BINGO} - should return corner win class`, () => {
        ...
        expect(result.length).to.equal(BINGO_CORNER_NUMBER_RANGE.end);
      });
      ...
      it(`${LottoType.BINGO} - should return diagonal win class`, () => {
        ...
        expect(result.length).to.equal(BINGO_DIAGONAL_NUMBER_RANGE.end);
      });

      it(`${LottoType.BINGO} - should return unknown win class (fallback)`, () => {
        const result = calculateNumberStats([1], LottoType.BINGO, false);
        expect(result.length).to.equal(BINGO_PRIMARY_NUMBER_RANGE.end);
      });

      it(`${LottoType.JOKKER} - should return primary range`, () => {
        const result = calculateNumberStats([1], LottoType.JOKKER);
        expect(result.length).to.equal(
          JOKKER_PRIMARY_NUMBER_RANGE.end - JOKKER_PRIMARY_NUMBER_RANGE.start + 1,
        );
      });

      it(`Unknown LottoType - should return empty array`, () => {
        const result = calculateNumberStats([1], 'UNKNOWN' as LottoType);
        expect(result.length).to.equal(0);
      });
    });
  });
  ...
  describe('countAllNumbersWithProbability', () => {
    it('should return empty array when input is empty', () => {
      const result = calculatePositionalNumberStats([], LottoType.KENO);
      expect(result).to.deepEqual([]);
    });

    it('should throw an error when sets have inconsistent lengths', () => {
      const sets = [[1, 2], [3]];
      expect(() => calculatePositionalNumberStats(sets, LottoType.KENO)).to.throw(
    ...
    it('should correctly compute frequencies and probabilities per position', () => {
      ...
      const result = calculatePositionalNumberStats(sets, LottoType.KENO);
    ...
    it('includes all digits in number range even if not present', () => {
      ...
      expect(pos0Stats.length).to.equal(KENO_PRIMARY_NUMBER_RANGE.end);
